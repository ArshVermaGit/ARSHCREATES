...
addNewProject() {
    ...
    if (!category || !['games', 'websites', 'apps'].includes(category)) {
        this.showNotification('Invalid category', 'error');
        return;
    }
    ...
    const overview = prompt('Enter project overview:');
    const image = prompt('Enter image path:');
    
    const newProject = {
        name,
        category: category === 'games' ? 'Action RPG' : 'Web Application',
        overview,
    ...
    if (category === 'games') {
        ...
        newProject.likes = 100;
    }

    addPortfolioItem(category, newProject);
    this.showNotification('Project added successfully!', 'success');
    this.loadData();
}

// Add this to the quick actions in init():
setupQuickActions() {
    const actions = document.querySelectorAll('.action-card');
    actions.forEach(action => {
        action.addEventListener('click', () => {
